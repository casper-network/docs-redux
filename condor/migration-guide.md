...
- Exchange systems are most likely interested in the JSON RPC API functionalities that Sidecar offers. You can see an example configuration that connects to a nodes binary port and exposes JSON RPC API here: https://github.com/casper-network/casper-sidecar/blob/dev/resources/example_configs/default_rpc_only_config.toml. You can see an explanation of the properties and guidance on setup here: https://github.com/casper-network/casper-sidecar/blob/dev/README.md#the-rpc-api-server
- When running a REST RPC JSON API server, Sidecar an open tcp/ip connection with the node. The node will discard idle connections, if you observe that Sidecar keeps disconnecting with the node and there are no network issues between them, you may need to adjust the `rpc_server.node_client.keepalive_timeout_ms` property.
- The node still exposes the SSE events (albeit the paths are different, instead of the specialized '/events/main', '/events/deploys' or '/events/sigs' we redirect all messages to '/events'. Also the structure of the events changed in a backwards-incompatible manner). But you can close the nodes SSE event enpoint to internet traffic and use Sidecar as an intermediary in SSE (Sidecar still needs to be able to connect to the SSE feed of the node). For more details on that please see: https://github.com/casper-network/casper-sidecar/blob/dev/README.md#sse-server-setup.
